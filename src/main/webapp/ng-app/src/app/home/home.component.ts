    ...
    template: `
        <form (ngSubmit)="submitTicketReference($event)" #ticketReferenceForm="ngForm">
            <div class="form-group">
                <md-input-container>
                    <input mdInput placeholder="Ticket reference" maxLength="255" class="form-control"
                           id="inputTicketReference" required
                           [(ngModel)]="ticketReferenceInput" name="inputTicketReference" #name="ngModel">
                </md-input-container>
                <button md-raised-button color="primary" type="submit" [disabled]="!ticketReferenceForm.form.valid"
                        id="submitTicketReferenceButton">OK
                </button>
            </div>
        </form>
    `, styles: [`
